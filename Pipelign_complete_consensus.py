...
def runType(x):
    x = x.upper()
    if x not in ['J','G']:
        raise argparse.ArgumentTypeError('%r must be either S, J or G' % (x,))
    return x
...
def longSeqAlignment(numClusters,mArgs,cDir,tFileName,zName):
    '''
...
def getArguments():
    ...
    parser.add_argument('-r', '--run', type=runType, help="Run either (S)erial/(J)oblib/(G)NU parallel version (default: G)", default='G')    
...
if __name__=="__main__":
    ...
    if mArgs.stage == 2:
        #longSeqAlignment(numClusters,mArgs.outFile,mArgs.keepBadSeqs,mArgs.thread,mArgs.mIterM,mArgs.mIterL,cDir,tFileName,zName)
        longSeqAlignment(numClusters,mArgs,cDir,tFileName,zName)
    ...
    if mArgs.stage == 3:
        ...
        if checkPresenceOfFile('frag.fas'):
            ...
            msg += '\tUnassigned fragments (if any) are written in <frag.noClusters.fas>\n'
        msg += '\nThank you for using Pipelign\n'
        sys.exit(msg)
    ...
    if mArgs.stage == 5:
        ...
        # only two clusters, merge the pair
            ...
            mergeConsensus(numClusters,'final.noOrphans.aln',mArgs.thread,'consensus.log',cDir,tFileName,zName)
        ...
        # check if <long.bad.fas> needs to be added to the final alignment
        if mArgs.keepBadSeqs and checkPresenceOfFile('long.bad.fas'):
            addLongSeqs('long.bad.fas','final.noBadSeq.aln','final.aln',mArgs.thread,mArgs.mIterL,'addBadSeq.log',cDir,tFileName,zName) 
        else:
            copyFile('final.noBadSeq.aln','final.aln')
